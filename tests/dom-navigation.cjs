...
(async()=>{
 ...
 const shield={status:{mode:'observe',stage:'normal',updated_at:new Date().toISOString(),enforcement:{active:false},metrics:{pps:12,mbps:2},layers:{}},config:{enabled:true,telemetry_enabled:true,low_cpu_mode:true,integration_guard_enabled:true,auto_stage:true,mode:'observe',policy:{management_ports:[22],rules:[],syn_ports:[],syn_rate:100}},events:[]};
 ...
 observers.forEach(o=>o.disconnect());await tick();dom.window.close();console.log('DOM regression: arrow, sidebar, top tabs, stable firewall controls, form persistence, renewal, feature transitions and native restore passed');
