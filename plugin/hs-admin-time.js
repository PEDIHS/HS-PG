(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      @media(prefers-reduced-motion:reduce){.hs-admin-time-gold,.hs-account-gold,#${ACCOUNT_CARD_ID},#${ACCOUNT_CARD_ID} .hs-account-glow,#${ACCOUNT_CARD_ID} .hs-account-shell,#${ACCOUNT_CARD_ID} .hs-account-progress-fill::after{animation:none!important}#${ACCOUNT_CARD_ID} .hs-account-progress-fill,#${ACCOUNT_CARD_ID} .hs-account-metric{transition:none!important}}
    `;
  }
  ...
  async function request(path, options = {}) {
    ...
    const response = await rawFetch(path, {credentials: 'same-origin', cache: 'no-store', ...rest, headers: authHeaders(supplied || {})});
  ...
  function setFieldStatus(field, text, kind = 'muted') {
    ...
    if (!el) return;
    setClass(el, `hs-admin-time-status min-h-4 text-[11px] ${kind === 'error' ? 'text-destructive' : kind === 'warn' ? 'text-amber-600 dark:text-amber-400' : kind === 'ok' ? 'text-green-600 dark:text-green-400' : 'text-muted-foreground'}`);
    setText(el, text);
  }
  ...
  function applyInfoToField(state, info) {
    ...
    if (!state.touched) {
      const cached = state.username ? draftCache.get(state.username) : null;
  ...
  async function hydrateState(form, state) {
    ...
    if (!state.editing) {
      ...
      const cached = draftCache.get(username);
      if (cached != null && !state.touched) {
        state.draft = cached;
        state.input.value = cached;
      }
      updateResetButton(state);
      return;
    }
  ...
  function mountField(form, state, dataItem) {
    ...
    if (state.draft != null && state.input.value !== state.draft) state.input.value = state.draft;
  ...
  function ensureField(form) {
    ...
    if (!state) {
      ...
      state.username = username; state.editing = editing; state.touched = false; state.dirty = false; state.draft = draftCache.get(username) ?? ''; state.loaded = false; state.hydrating = false; state.supported = true; state.info = null;
    } else {
      state.username = username; state.editing = editing;
    }
  ...
  function beginPending(form, event) {
    ...
    const item = {dialog: form.closest('[role="dialog"]') || form.parentElement, username, days, editing: state.editing, startedAt: Date.now(), successSeen: false, finalized: false, existingBefore: state.editing ? Promise.resolve(true) : hsApi(`/admin-time/by-username/${encodeURIComponent(username)}`).then(() => true).catch(error => error.status === 404 ? false : null)};
    pending.add(item);
  ...
  function markNativeSuccess() {
    const candidates = [...pending].filter(item => Date.now() - item.startedAt < 20000 && !item.finalized).sort((a, b) => b.startedAt - a.startedAt);
    if (candidates[0]) candidates[0].successSeen = true;
  }
  function processPending() {
    for (const item of [...pending]) {
      if (item.finalized) continue;
      if (Date.now() - item.startedAt > 25000) { pending.delete(item); continue; }
  ...
  function accountState() {
    ...
    let state = 'active', label = tr('Active', 'فعال');
    if (timeExpired) { state = 'expired'; label = tr('Time expired', 'زمان تمام شده'); }
    else if (trafficExpired) { state = 'expired'; label = tr('Traffic exhausted', 'حجم تمام شده'); }
    else if (nativeStatus === 'disabled') { state = 'expired'; label = tr('Disabled', 'غیرفعال'); }
    else if (nativeStatus === 'limited') { state = 'warning'; label = tr('Limited', 'محدود'); }
    else if (warning) { state = 'warning'; label = tr('Ending soon', 'رو به پایان'); }
  ...
  function accountCardMarkup() {
    return `<div class="hs-account-shell bg-card relative overflow-hidden rounded-xl border p-4 shadow-sm transition-all duration-300 hover:shadow-lg sm:p-5 lg:p-6"><div class="hs-account-glow pointer-events-none absolute -right-16 -top-24 h-64 w-64 rounded-full bg-primary/20 blur-3xl"></div><div class="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-amber-400/55 to-transparent"></div><div class="relative z-10"><div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div class="flex min-w-0 items-center gap-3"><div class="bg-primary/10 text-primary flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-primary/10"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5"><path d="M4 19V8"/><path d="M10 19V5"/><path d="M16 19v-8"/><path d="M22 19V3"/><path d="M2 19h22"/></svg></div><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><h3 data-hs-account-title class="text-base font-semibold sm:text-lg"></h3><span class="hs-account-gold text-[10px] tracking-wider">HS</span><span data-hs-account-status class="rounded-md border px-2 py-0.5 text-[10px] font-medium"></span></div><p data-hs-account-subtitle class="text-muted-foreground mt-1 text-xs"></p></div></div><div class="text-muted-foreground flex items-center gap-1.5 text-[11px]"><span class="relative flex h-2 w-2"><span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-500 opacity-30"></span><span class="relative inline-flex h-2 w-2 rounded-full bg-green-500"></span></span><span>${tr('Live', 'زنده')}</span></div></div><div class="grid grid-cols-1 gap-3 sm:grid-cols-3"><div class="hs-account-metric bg-background/65 rounded-xl border p-4"><div class="text-muted-foreground mb-2 text-[11px] font-medium">${tr('Available volume', 'حجم قابل استفاده')}</div><div data-hs-account-total class="hs-account-value text-xl font-bold tracking-tight sm:text-2xl" dir="ltr"></div><div data-hs-account-total-caption class="text-muted-foreground mt-1 text-[10px]"></div></div><div class="hs-account-metric bg-background/65 rounded-xl border p-4"><div class="text-muted-foreground mb-2 text-[11px] font-medium">${tr('Remaining volume', 'حجم باقی‌مانده')}</div><div data-hs-account-remaining class="hs-account-value text-xl font-bold tracking-tight sm:text-2xl" dir="ltr"></div><div data-hs-account-used class="text-muted-foreground mt-1 text-[10px]"></div></div><div class="hs-account-metric bg-background/65 rounded-xl border p-4"><div class="text-muted-foreground mb-2 text-[11px] font-medium">${tr('Remaining time', 'زمان باقی‌مانده')}</div><div data-hs-account-time class="hs-account-value text-xl font-bold tracking-tight sm:text-2xl" dir="ltr"></div><div data-hs-account-expiry class="text-muted-foreground mt-1 text-[10px]"></div></div></div><div class="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2"><div class="bg-muted/25 rounded-xl border p-3.5 sm:p-4"><div class="mb-2 flex items-center justify-between gap-3 text-[11px]"><span class="text-muted-foreground">${tr('Traffic usage', 'مصرف حجم')}</span><span data-hs-account-traffic-percent class="font-semibold" dir="ltr"></span></div><div class="bg-muted h-2.5 overflow-hidden rounded-full"><div data-hs-account-traffic-fill class="hs-account-progress-fill bg-primary h-full rounded-full" style="width:0%"></div></div></div><div class="bg-muted/25 rounded-xl border p-3.5 sm:p-4"><div class="mb-2 flex items-center justify-between gap-3 text-[11px]"><span class="text-muted-foreground">${tr('Account lifetime', 'زمان اعتبار اکانت')}</span><span data-hs-account-time-percent class="font-semibold" dir="ltr"></span></div><div class="bg-muted h-2.5 overflow-hidden rounded-full"><div data-hs-account-time-fill class="hs-account-progress-fill h-full rounded-full bg-amber-500" style="width:0%"></div></div></div></div></div></div>`;
  }
  function ensureAccountCard() {
    const shouldShow = !!account && !account?.role?.is_owner && isDashboardRoute();
    let host = document.getElementById(ACCOUNT_CARD_ID);
    if (!shouldShow) { host?.remove(); return null; }
    const stack = dashboardStack();
    if (!stack) return null;
    if (!host) {
      host = document.createElement('div');
      host.id = ACCOUNT_CARD_ID;
      host.dataset.hsOwned = '1';
  ...
  function renderAccountCard() {
    ...
    if (host.dataset.state !== s.state) host.dataset.state = s.state;
    setText(host.querySelector('[data-hs-account-title]'), tr('Your Account', 'وضعیت اکانت شما'));
    setText(host.querySelector('[data-hs-account-subtitle]'), account.username ? tr(`Live allowance for ${account.username}`, `نمایش زنده اعتبار و حجم ${account.username}`) : tr('Live account allowance', 'نمایش زنده اعتبار اکانت'));
    const badge = host.querySelector('[data-hs-account-status]');
    setText(badge, s.label);
    setClass(badge, `rounded-md border px-2 py-0.5 text-[10px] font-medium ${s.state === 'expired' ? 'border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-400' : s.state === 'warning' ? 'border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'border-green-500/30 bg-green-500/10 text-green-600 dark:text-green-400'}`);
    setText(host.querySelector('[data-hs-account-total]'), s.limit > 0 ? formatBytes(s.limit) : '∞');
    setText(host.querySelector('[data-hs-account-total-caption]'), s.limit > 0 ? tr('Total assigned traffic', 'کل حجم اختصاص داده‌شده') : tr('Unlimited traffic', 'حجم نامحدود'));
    setText(host.querySelector('[data-hs-account-remaining]'), s.limit > 0 ? formatBytes(s.remainingTraffic) : '∞');
    setText(host.querySelector('[data-hs-account-used]'), `${tr('Used', 'مصرف‌شده')}: ${formatBytes(s.used)}`);
    setText(host.querySelector('[data-hs-account-time]'), !s.timeEnabled ? tr('Disabled', 'غیرفعال') : !s.timeConfigured ? '∞' : formatCompactTime(s.timeRemaining));
    setText(host.querySelector('[data-hs-account-expiry]'), !s.timeEnabled ? tr('Time limit feature is off', 'محدودیت زمانی غیرفعال است') : !s.timeConfigured ? tr('Unlimited time', 'بدون محدودیت زمانی') : selfTime?.expires_at ? `${tr('Expires', 'پایان')}: ${formatDate(selfTime.expires_at)}` : '');
    const trafficFill = host.querySelector('[data-hs-account-traffic-fill]');
    setWidth(trafficFill, s.limit > 0 ? s.volumePercentUsed : 0);
    setClass(trafficFill, `hs-account-progress-fill h-full rounded-full ${s.limit > 0 && s.volumePercentRemaining <= 10 ? 'bg-red-500' : s.limit > 0 && s.volumePercentRemaining <= 25 ? 'bg-amber-500' : 'bg-primary'}`);
    setText(host.querySelector('[data-hs-account-traffic-percent]'), s.limit > 0 ? `${s.volumePercentUsed.toFixed(1)}%` : tr('Unlimited', 'نامحدود'));
    const timeFill = host.querySelector('[data-hs-account-time-fill]');
    setWidth(timeFill, s.timeConfigured ? s.timePercentRemaining : 100);
    setClass(timeFill, `hs-account-progress-fill h-full rounded-full ${s.timeConfigured && s.timePercentRemaining <= 10 ? 'bg-red-500' : 'bg-amber-500'}`);
    setText(host.querySelector('[data-hs-account-time-percent]'), s.timeConfigured ? `${s.timePercentRemaining.toFixed(1)}%` : tr('Unlimited', 'نامحدود'));
  }

  async function refreshAccount(force = false) {
    if (accountRefreshBusy) return;
    if (!force && Date.now() - lastAccountFetch < 10000) return;
    accountRefreshBusy = true;
    try {
      const [adminResult, timeResult] = await Promise.allSettled([request('/api/admin'), hsApi('/admin-time/me')]);
  ...
  async function refreshFeature() {
    try {
      ...
      if (error.status === 401 || error.status === 403) ownerAccess = false;
    }
    ensureFeatureCard();
  ...
  function isHsOwnedNode(node) {
    const el = node instanceof Element ? node : node?.parentElement;
    return !!el?.closest?.(`#${ACCOUNT_CARD_ID}, #${FEATURE_CARD_ID}, .${FIELD_CLASS}, [data-hs-owned="1"]`);
  }
  function inspectAddedNode(node) {
    if (!(node instanceof Element) || isHsOwnedNode(node)) return;
  ...
  function start() {
    ...
    window.addEventListener('hashchange', () => { scheduleSync(60); refreshAccount(true); });
    window.addEventListener('popstate', () => { scheduleSync(60); refreshAccount(true); });
    ...
    observer = new MutationObserver(mutations => {
      ...
      for (const mutation of mutations) {
        if (isHsOwnedNode(mutation.target)) continue;
        nativeMutation = true;
        for (const node of mutation.addedNodes) inspectAddedNode(node);
      }
      if (nativeMutation) scheduleSync(60);
    });
    ...
    setInterval(() => scheduleSync(0), 2000);
  }
  ...
  window.HSAdminTime = {
    version: VERSION,
    refresh: async () => { await Promise.allSettled([refreshFeature(), refreshAccount(true)]); scheduleSync(0); },
    get enabled() { return enabled; },
    diagnostics: () => ({version: VERSION, enabled, ownerAccess, route: routePath(), fields: document.querySelectorAll(`.${FIELD_CLASS}`).length, forms: adminForms().length, account: account?.username || null, owner: !!account?.role?.is_owner, dataLimit: account?.data_limit ?? null, usedTraffic: account?.used_traffic ?? null, timeConfigured: !!selfTime?.configured, timeEnabled: selfTime?.enabled, accountCard: !!document.getElementById(ACCOUNT_CARD_ID), pending: pending.size, accountRefreshBusy}),
  };
})();
