...
for f in "$PATCHER" "$BACKUP_PATCHER" "$ADMIN_JS" "$TAB_FIX_JS" "$NODE_PRO_JS" "$BACKUP_JS" "$API_ADDON" "$BACKUP_API" "$RUNTIME"; do
  [[ -s "$f" ]] || { warn "missing $f"; exit 1; }
done
mkdir -p "$DATA_DIR"

inject_loaders_host(){
  local html="$1" admin_version tab_version node_version backup_version
  [[ -f "$html" ]] || return 0
  admin_version="$(sha12 "$ADMIN_JS")"
  tab_version="$(sha12 "$TAB_FIX_JS")"
  node_version="$(sha12 "$NODE_PRO_JS")"
  backup_version="$(sha12 "$BACKUP_JS")"
  python3 - "$html" "$LOADER_MARKER" "$admin_version" "$TAB_FIX_MARKER" "$tab_version" "$NODE_PRO_MARKER" "$node_version" "$BACKUP_MARKER" "$backup_version" "$LEGACY_NODE_IP_MARKER" <<'PY'
...
inject_loaders_container(){
  local cid="$1" html="$2" admin_version tab_version node_version backup_version
  admin_version="$(sha12 "$ADMIN_JS")"
  tab_version="$(sha12 "$TAB_FIX_JS")"
  node_version="$(sha12 "$NODE_PRO_JS")"
  backup_version="$(sha12 "$BACKUP_JS")"
  docker exec "$cid" test -f "$html" >/dev/null 2>&1 || return 0
  docker exec -i "$cid" python3 - "$html" "$LOADER_MARKER" "$admin_version" "$TAB_FIX_MARKER" "$tab_version" "$NODE_PRO_MARKER" "$node_version" "$BACKUP_MARKER" "$backup_version" "$LEGACY_NODE_IP_MARKER" <<'PY'
from pathlib import Path
import re, sys
p=Path(sys.argv[1]); old=p.read_text(encoding='utf-8')
entries=[
...
main(){
  ...
  log "integration complete; HS tab, Node PRO, Web Backup and backend hooks are installed"
}
