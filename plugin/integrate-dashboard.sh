...
for f in "$PATCHER" "$ADMIN_JS" "$API_ADDON" "$RUNTIME"; do [[ -s "$f" ]] || { warn "missing $f"; exit 1; }; done
mkdir -p "$DATA_DIR"

inject_loader_host(){
  local html="$1" version; [[ -f "$html" ]] || return 0; version="$(sha12 "$ADMIN_JS")"
  python3 - "$html" "$LOADER_MARKER" "$version" <<'PY'
from pathlib import Path
import re, sys
p=Path(sys.argv[1]); marker=sys.argv[2]; version=sys.argv[3]; old=p.read_text(encoding='utf-8')
tag=f'<script id="{marker}" src="/statics/hs-plugin.js?v={version}" defer></script>'
...
inject_loader_container(){
  local cid="$1" html="$2" version; version="$(sha12 "$ADMIN_JS")"
  docker exec "$cid" test -f "$html" >/dev/null 2>&1 || return 0
  docker exec -i "$cid" python3 - "$html" "$LOADER_MARKER" "$version" <<'PY'
from pathlib import Path
import re, sys
p=Path(sys.argv[1]); marker=sys.argv[2]; version=sys.argv[3]; old=p.read_text(encoding='utf-8')
tag=f'<script id="{marker}" src="/statics/hs-plugin.js?v={version}" defer></script>'
...
find_host_app(){
  local p
  for p in "$PASARGUARD_ROOT/app" "$PASARGUARD_ROOT/panel/app"; do [[ -f "$p/routers/__init__.py" ]] && { printf '%s\n' "$p"; return; }; done
}
find_host_build(){
  local p
  for p in "$PASARGUARD_ROOT/dashboard/build" "$PASARGUARD_ROOT/panel/dashboard/build"; do [[ -f "$p/index.html" ]] && { printf '%s\n' "$p"; return; }; done
}
...
integrate_host(){
  ...
  if [[ -n "$build" ]]; then
    mkdir -p "$build/statics"; install -m 0644 "$ADMIN_JS" "$build/statics/hs-plugin.js"
...
main(){
  integrate_host || true
  local ids cid count=0; ids="$(container_ids)"
  while read -r cid; do [[ -n "$cid" ]] || continue; integrate_container "$cid"; count=$((count+1)); done <<<"$ids"
  if [[ $count -eq 0 && -z "$(find_host_app || true)" ]]; then warn "no active PasarGuard installation found"; exit 2; fi
  log "integration complete; Python hooks take effect after the next normal PasarGuard process restart"
}
main "$@"
