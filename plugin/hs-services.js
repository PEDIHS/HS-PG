...
`;document.head.appendChild(style);
  ...
  async function nodes(body,data){
    ...
    body.innerHTML=`<div class="hs-cert-summary"><div><span class="hs-s-badge">HS NODE BRIDGE</span><h3>Secure node control plane</h3><p>Local Nodes are managed directly by the panel. Remote Nodes use one outbound HTTPS Bridge for Fair Use, Certbot inventory, updates and future HS capabilities. No extra inbound management port is required.</p></div><div><strong>${online}/${list.length}</strong><span>Bridge online</span></div><div><strong>${list.filter(t=>t.enrolled).length}</strong><span>Managed</span></div><div><strong>${list.filter(t=>t.capabilities?.fair_rate_limit).length}</strong><span>Fair Core ACK</span></div></div><form class="hs-s-card" data-node-register style="margin-bottom:12px"><div class="hs-cert-top"><span class="hs-s-badge">ONE-STEP ENROLLMENT</span></div><h3>Register PasarGuard Node + HS Bridge</h3><p>For a remote server, run PasarGuard Node first, then use this one-line installer. Nodes running on the panel server are detected automatically and need no Bridge install.</p><div class="hs-s-grid"><label>Name<input name="name" required placeholder="Poland"></label><label>Domain or IP<input name="address" required placeholder="node.example.com"></label><label>Core<select name="core_id" required>${cores.map(c=>`<option value="${esc(c.id)}">${esc(c.name)}</option>`).join('')}</select></label><label>Usage Ratio<input name="usage_coefficient" type="number" min="0" max="100" step="0.1" value="1"></label></div><details><summary>Connection settings</summary><div class="hs-s-grid" style="margin-top:12px"><label>gRPC port<input name="port" type="number" value="62050" min="1024" max="65535"></label><label>Node API port<input name="api_port" type="number" value="62051" min="1024" max="65535"></label><label>Keep alive<input name="keep_alive" type="number" value="30" min="0" max="3600"></label><label>Timeout / internal<input name="timeouts" value="30,60" pattern="[0-9]+,[0-9]+"></label></div></details><div class="hs-s-actions" style="margin-top:14px"><button type="submit">Generate one-step installer</button></div><div data-new-node-install></div></form><div class="hs-s-grid">${list.map(t=>{
      ...
      const local=!!t.local;
      const state=local?(t.online?'Local · Online':'Local · Offline'):(t.online?'Online':t.enrolled?'Offline':'Not installed');
