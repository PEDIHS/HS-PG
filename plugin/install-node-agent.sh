...
curl -fsSL --retry 3 --connect-timeout 15 --max-time 120 "https://raw.githubusercontent.com/PEDIHS/HS-PG/$REF/plugin/retire-firewall.sh" -o "$agent_tmp/retire-firewall.sh"
bash -n "$agent_tmp/retire-firewall.sh"
python3 -m py_compile "$agent_tmp"/*.py
systemctl stop hs-services-agent.service >/dev/null 2>&1 || true
bash "$agent_tmp/retire-firewall.sh"
...
[Unit]
Description=HS node certificate and Telegram service agent
