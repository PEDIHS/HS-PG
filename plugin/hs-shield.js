(() => {
  ...
  const css = `
    @keyframes hsShieldPulse{0%,100%{transform:scale(.98);opacity:.7}50%{transform:scale(1.035);opacity:1}}
    ...
    #${ROOT_ID} .hs-chip strong{font-weight:700;color:white}
    ...
    #${ROOT_ID} .hs-layers{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0;border-top:0}
    ...
    #${ROOT_ID} .hs-good{color:#22c55e}.hs-warn{color:#f59e0b}.hs-bad{color:#ef4444}
    ...
    @media(max-width:1100px){#${ROOT_ID} .hs-grid{grid-template-columns:repeat(2,1fr)}#${ROOT_ID} .hs-layers{grid-template-columns:repeat(3,1fr)}#${ROOT_ID} .hs-layer{border-bottom:1px solid hsl(var(--border))}#${ROOT_ID} .hs-main{grid-template-columns:1fr}}
    ...
    @media(prefers-reduced-motion:reduce){#${ROOT_ID} .hs-orb,#${ROOT_ID} .hs-hero:before,#${ROOT_ID} .hs-stage.active:after,.hs-shield-nav-dot.attack{animation:none}}
  `;
  ...
  async function api(path,options={}){
    ...
    const response=await rawFetch(`/api/hs-shield${path}`,{credentials:'same-origin',...rest,headers:authHeaders(headers)});
  ...
  function deactivate(){
    ...
    root?.remove();
    restoreNativeActive();
    const button=document.querySelector(`#${NAV_ID} [data-sidebar="menu-button"]`);if(button)button.dataset.active='false';
  }
  ...
  function updateNav(stage='unknown'){
    const dot=document.querySelector(`#${NAV_ID} .hs-shield-nav-dot`);
    if(dot){dot.className='hs-shield-nav-dot '+(stage==='attack'?'attack':stage==='elevated'?'warn':['normal','standby'].includes(stage)?'ok':'')}
  ...
  function ensureNav(){
    ...
    if(document.getElementById(NAV_ID)){updateNav(latest?.status?.stage);return}
    ...
    li.appendChild(button);anchor.after(li);updateNav(latest?.status?.stage);
  }
  ...
  function render(data){
    latest=data;
    const s=data?.status||{};const m=s.metrics||{};const layers=s.layers||{};const events=Array.isArray(data?.events)?data.events:[];const stage=s.stage||'starting';const origin=s.origin||{};
    ...
    const reason=(Array.isArray(s.stage_reasons)&&s.stage_reasons[0])||'Waiting for telemetry';
    ...
    root.innerHTML=`<div class="hs-wrap">
      <section class="hs-hero">
        <div class="hs-hero-grid"><div><div class="hs-kicker">HS Shield • v${VERSION}</div><h2>Panel Firewall Control Plane</h2><div class="hs-muted">Live, multi-stage protection telemetry with a fail-open rollout. Phase 1 observes only and never changes panel traffic.</div><div class="hs-chip-row"><span class="hs-chip">Stage <strong>${esc(stageLabel(stage))}</strong></span><span class="hs-chip">Mode <strong>Observe</strong></span><span class="hs-chip">Origin <strong>${esc(origin.state||'unknown')}</strong></span><span class="hs-chip">Layers <strong>${Number(s.layers_ready)||0}/${Number(s.layers_total)||0}</strong></span></div></div><div class="hs-orb">${shieldIcon()}</div></div>
      </section>
      ...
      <div class="hs-grid">
        <div class="hs-card"><div class="hs-label">Incoming packet rate</div><div class="hs-value">${fmtRate(m.pps)} <small style="font-size:.62em;font-weight:600">pps</small></div><div class="hs-sub">Adaptive baseline ${fmtRate(s.baseline?.pps||0)} pps</div></div>
        <div class="hs-card"><div class="hs-label">Network throughput</div><div class="hs-value">${Number(m.mbps||0).toFixed(1)} <small style="font-size:.62em;font-weight:600">Mbps</small></div><div class="hs-sub">Host-wide observation, no packet interception</div></div>
        <div class="hs-card"><div class="hs-label">TCP SYN-RECV</div><div class="hs-value">${Math.round(Number(m.syn_recv)||0)}</div><div class="hs-sub">Useful signal for connection-flood escalation</div></div>
        <div class="hs-card"><div class="hs-label">Established TCP</div><div class="hs-value">${Math.round(Number(m.established)||0)}</div><div class="hs-sub">Current host connection pressure</div></div>
      </div>
      <section class="hs-section"><div class="hs-section-head"><h3>Protection layers</h3><span class="hs-pill">live health</span></div><div class="hs-layers">${Object.keys(layers).length?Object.entries(layers).map(([key,val])=>`<div class="hs-layer"><div class="hs-layer-name">${esc(layerName(key))}</div><div class="hs-layer-state"><i class="hs-dot ${val?.active?'on':'off'}"></i>${val?.active?'Active':'Not active'}</div><div class="hs-sub">${esc(val?.role||'layer')}</div></div>`).join(''):'<div class="hs-empty">Waiting for layer discovery…</div>'}</div></section>
      <div class="hs-main">
        <section class="hs-section hs-events"><div class="hs-section-head"><h3>Security timeline</h3><button class="hs-refresh" id="hs-shield-refresh">Refresh</button></div>${events.length?events.map(ev=>`<div class="hs-event"><div class="hs-event-time">${esc(fmtTime(ev.at))}</div><i class="hs-event-dot ${esc(ev.kind||'')}"></i><div><div class="hs-event-title">${esc(ev.message||ev.kind||'Event')}</div><div class="hs-sub">Stage: ${esc(stageLabel(ev.stage||'unknown'))}</div></div></div>`).join(''):'<div class="hs-empty">No security events recorded yet.</div>'}</section>
        <section class="hs-section"><div class="hs-section-head"><h3>Safety & exposure</h3><span class="hs-pill">fail-open</span></div><div class="hs-side"><div class="hs-safe"><strong>Traffic is untouched</strong>Observe mode cannot add firewall rules, restart PasarGuard, or change Docker networking. Enforcement is locked until health-check + rollback are implemented.</div><div class="hs-row"><span>Current reason</span><b>${esc(reason)}</b></div><div class="hs-row"><span>Origin exposure</span><b class="${originClass}">${esc(origin.state||'unknown')}</b></div><div class="hs-row"><span>Public bindings</span><b>${Array.isArray(origin.public_bindings)?origin.public_bindings.length:0}</b></div><div class="hs-row"><span>Traffic modified</span><b class="hs-good">No</b></div><div class="hs-row"><span>Fail-open</span><b class="hs-good">Enabled</b></div><div class="hs-row"><span>Last telemetry</span><b>${esc(fmtTime(s.updated_at))}</b></div></div></section>
      </div>
    </div>`;
  ...
  async function refresh(force=false){
    try{const data=await api('/status');allowed=true;latest=data;ensureNav();if(active||force)render(data)}catch(error){if(error.status===401||error.status===403){allowed=false;deactivate();document.getElementById(NAV_ID)?.remove()}else{console.warn('[HS Shield] status failed',error)}}
  }
  function startPolling(){stopPolling();pollTimer=setInterval(()=>refresh(false),2000)}
  function stopPolling(){if(pollTimer){clearInterval(pollTimer);pollTimer=null}}
  async function activate(){
    if(!latest)await refresh(false);if(!allowed)return;
    const other=document.getElementById('hs-plugin-root');if(other){const outlet=other.parentElement;[...outlet.children].forEach(el=>{if(el.hasAttribute('data-hs-outlet-display')){el.style.display=el.getAttribute('data-hs-outlet-display')||'';el.removeAttribute('data-hs-outlet-display')}});other.remove()}
    active=true;suppressNativeActive();updateNav(latest?.status?.stage);if(render(latest))startPolling();else active=false;
  }

  function watchNavigation(){
    document.addEventListener('click',event=>{if(!active)return;const target=event.target instanceof Element?event.target.closest('a,[data-sidebar="menu-button"]'):null;if(!target||target.closest(`#${NAV_ID}`))return;setTimeout(()=>{if(!document.getElementById(ROOT_ID))return;deactivate()},0)},true);
  }

  function boot(){
    injectStyle();watchNavigation();refresh(false);
