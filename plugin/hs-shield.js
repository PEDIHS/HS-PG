(() => {
  ...
  function renderSection(root){
    ...
    if(section==='rules'){
      ...
      content.querySelector('form').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);rows.push({id:crypto.randomUUID(),action:f.get('action'),source:f.get('source'),protocol:f.get('protocol'),port:f.get('port')?Number(f.get('port')):null,enabled:true});draw();e.currentTarget.reset();};
